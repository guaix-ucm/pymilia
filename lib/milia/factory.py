...
def Flrw(hubble, matter, vacuum):

    if matter == 0 and vacuum == 0:
        return Flrw_OM_OV_0(hubble)

    if matter == 0 and vacuum == 1:
        return Flrw_OM_DS(hubble)
    if matter == 0 and 0 < vacuum < 1:
        return Flrw_OM(hubble, vacuum)

    if matter == 1 and vacuum == 0:
        return Flrw_OV_EDS(hubble)
    if matter < 1 and vacuum == 0:
        return Flrw_OV_1(hubble, matter)
    if matter > 1 and vacuum == 0:
        return Flrw_OV_2(hubble, matter)

    if matter + vacuum == 1:
        return Flrw_OM_OV_1(hubble, matter, vacuum)
    
    ok = 1 - matter - vacuum
    crit = -13.5 * matter**2 * vacuum / ok**3

    if crit == 2:
        return FlrwA2_1(hubble, matter, vacuum)
    elif 0 < crit < 2:
        return FlrwA2_2(hubble, matter, vacuum)
    elif crit > 2 or crit < 0:
        return FlrwA1(hubble, matter, vacuum)

    return FlrwNonFlat(hubble, matter, vacuum)

class FlrwBase(object):
    ...
    def __str__(self):
        return 'milia.Flrw(hubble=%f, matter=%f, vacuum=%f)' % (self.hubble, self.om, self.ov)
...
class FlrwFlat(FlrwBase):
    def __init__(self, hubble, matter, vacuum):
        super(FlrwFlat, self).__init__(hubble, matter, vacuum)
...
class Flrw_OV_EDS(FlrwFlat):
    def __init__(self, hubble):
        super(Flrw_OV_EDS, self).__init__(hubble, 1.0, 0.0)

    def dl(self, z):
        return self.hubble_radius * 2 * (1 + z - math.sqrt(1 + z))
    
    def age(self, z):
        return self.hubble_time * 2 / (3 * (1 + z) * math.sqrt(1 + z))

class Flrw_OM_DS(FlrwFlat):
    def __init__(self, hubble):
        super(Flrw_OM_DS, self).__init__(hubble, 0.0, 1.0)

    def dl(self, z):
        return self.hubble_radius * z * (z + 1)

class Flrw_OM_OV_1(FlrwFlat): # OM_OV_1
    def __init__(self, hubble, matter, vacuum):
        super(Flrw_OM_OV_1, self).__init__(hubble, matter, vacuum)
    ...
    def dl(self, z):
        ...
        return self.hubble_radius * (1 + z) / self.factor * (ellipkinc(self.phi0, self.k) - ellipkinc(phi, self.k))

    def age(self, z):
        return self.hubble_time * 2. / (3. * math.sqrt(1 - m_om)) * math.asinh(sqrt((1. / m_om - 1) / (1 + z)**3))

class FlrwNonFlat(FlrwBase):
    def __init__(self, hubble, matter, vacuum):
        super(FlrwNonFlat, self).__init__(hubble, matter, vacuum)

    def dc(self, z):
        dm = self.dm(z) / self.hubble_radius
        return self.hubble_radius * asinc(self.kap, self.sqok, dm)

    def vol(self, z):
        dm = self.dm(z) / self.hubble_radius
        x = (dm * math.sqrt(1 + self.ok * dm * dm) - asinc(self.kap, self.sqok, dm)) / (2 * self.ok)
        return self.hubble_radius**3 * x

class Flrw_OM_OV_0(FlrwNonFlat):
    def __init__(self, hubble):
        super(Flrw_OM_OV_0, self).__init__(hubble, 0.0, 0.0)

    def dl(self, z):
        return self.hubble_radius * 0.5 * z * (z + 2)

    def age(self, z):
        return self.hubble_time / (1 + z)

class Flrw_OM(FlrwNonFlat):
    def __init__(self, hubble, vacuum):
        super(Flrw_OM, self).__init__(hubble, 0.0, vacuum)

    def dl(self, z):
        return self.hubble_radius * ((1 + z) / self.ov) * (1 + z - math.sqrt(self.ov + (1 - self.ov) * (1 + z)*(1+z)))

class Flrw_OV(FlrwNonFlat):
    def __init__(self, hubble, matter):
        super(Flrw_OV, self).__init__(hubble, matter, 0.0)

    def dl(self, z):
        return self.hubble_radius * 2 * ((2 - self.om * (1 - z) - (2 - self.om) * math.sqrt(1 + self.om * z))) / (self.om * self.om)

class Flrw_OV_1(Flrw_OV):
    def __init__(self, hubble, matter):
        super(Flrw_OV_1, self).__init__(hubble, matter)

class Flrw_OV_2(Flrw_OV):
    def __init__(self, hubble, matter):
        super(Flrw_OV_2, self).__init__(hubble, matter)

class FlrwA(FlrwNonFlat):
    def __init__(self, hubble, matter, vacuum):
        super(FlrwA, self).__init__(hubble, matter, vacuum)
        self.crit = -13.5 * matter**2 * vacuum / self.ok**3

class FlrwA1(FlrwA):
    def __init__(self, hubble, matter, vacuum):
        super(FlrwA1, self).__init__(hubble, matter, vacuum)
    ...
    def dl(self, z):
        ...
        a = (1 + z) / self.sqok * sinc(self.kap, 1.0, g * (ellipkinc(phi0, k) - ellipkinc(phi, k)))
        return a * self.hubble_radius

class FlrwA2(FlrwA):
    def __init__(self, hubble, matter, vacuum):
        super(FlrwA2, self).__init__(hubble, matter, vacuum)
    ...
    def dl(self, z):
        ...
        ff = (1. + z) / self.sqok * math.sin(g * (ellipkinc(phi0, k) - ellipkinc(phi, k)))
        return self.hubble_radius * ff

class FlrwA2_1(FlrwA2):
    def __init__(self, hubble, matter, vacuum):
        super(FlrwA2_1, self).__init__(hubble, matter, vacuum)

class FlrwA2_2(FlrwA2):
    def __init__(self, hubble, matter, vacuum):
        super(FlrwA2_2, self).__init__(hubble, matter, vacuum)
